...
class Move():
    ...
    def draw(self, c, frate=10000):
        zoff = -4.7 - (1.3*c/52)
        zoff = round(zoff, 1)
        f = [f"G1X125Y0Z-1F{frate}", "G04P0", "S1000M03", "$1=255", "G04P0", f"G1X125Y0Z{zoff}F200", "G04P0", "G04P1", "G04P0",
    ...
    def place(self, coord2, coord1=[0, 0], frate=10000):
        coord1[1] = coord1[1]
        coord2[1] = coord2[1]
        f = [f"G01X{coord1[0]}Y{coord1[1]}Z-1F{frate}", "S1000M03", "$1=255",
             ...
             "M05", "$1=0", "G04P1", f"G0X{coord2[0]}Y{coord2[1]}Z-1", f"G1X125Y0Z-1F{frate}"]
        # Stream g-code to grbl
        for line in f:
            l = line.strip()  # Strip all EOL characters for consistency
            print('Sending: ' + l)
    ...
    def discard(self, stack, frate=10000):
        for c in stack:
            c[1] = c[1]
            f = [f"G01X{c[0]}Y{c[1]}Z-1F{frate}", "S1000M03", "$1=255", f"G01X{c[0]}Y{c[1]}Z-6.0F200",
                 "G04P2", f"G01X{c[0]}Y{c[1]}Z0F200", "G0X250Y-15Z0", "M05", "$1=0", "G04P2"]
            for line in f:
                l = line.strip()  # Strip all EOL characters for consistency
                print('Sending: ' + l)
                self.s.write(str.encode(l + '\n'))  # Send g-code block to grbl
